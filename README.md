...
|-- data/
|   `-- HY202103/                     원본 측정 데이터 (wafer 별 폴더,
|                                      그 안에 측정 날짜 폴더와 XML)
|       |-- D07/<YYYYMMDD_HHMMSS>/*.xml
|       |-- D08/<YYYYMMDD_HHMMSS>/*.xml
|       |-- D23/<YYYYMMDD_HHMMSS>/*.xml
|       `-- D24/<YYYYMMDD_HHMMSS>/*.xml
|-- doc/                              방법론 그림 (강의 자료 등)
`-- res/
    |-- csv/                          최신 실행의 추적 CSV/XLSX
    |-- figures/                      최신 실행의 추적 PNG (메인 파이프라인)
    |-- diagnostics/                  investigate.py / sensitivity_test.py 의 그림
